...
<head>
    ...
    #donutchart{position : absolute; top : 150px; left : 380px;}
    #bs-example-navbar-collapse-11{position : absolute; top : 150px; left : 770px; z-index:1;}
    #section { position : absolute; top : 150px; left : 1537px;}
    img	 { position : absolute; top :150px; left :900px;}
    </style>
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ...
          ['<%=sbd.get(0).getBseatcode()%>', <%=sbd.get(0).getBcount()%>],
          ['<%=sbd.get(1).getBseatcode()%>', <%=sbd.get(1).getBcount()%>],
          <%-- ['<%=sbd.get(2).getBseatcode()%>', <%=sbd.get(2).getBcount()%>],
          ['<%=sbd.get(3).getBseatcode()%>', <%=sbd.get(3).getBcount()%>],
          ['<%=sbd.get(4).getBseatcode()%>', <%=sbd.get(4).getBcount()%>],
          ['<%=sbd.get(5).getBseatcode()%>', <%=sbd.get(5).getBcount()%>], --%>
        ]);
...
<body>
    ...
    <div class="container">
        <div class="row">
      <div id="donutchart" style="width: 550px; height: 615px;"></div>
        </div>
        <!-- /.row -->

    </div>
    <!-- /.container -->
<img src = "A.png" width = 500px height = 500px>
